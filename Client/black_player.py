...
def start_game():
    response = requests.get(chess_api + 'start-game')
...
def accept_challenge():
    global my_turn
    data = {'challenger_id': 'QjLu5IhCQGuGQDjOAAAB'}
    response = requests.get(chess_api + 'accept-challenge', json=data, headers=api_headers)
    my_turn = False
    valid_move_data = response.json()
    return convert_data.convert_valid_moves_data(valid_move_data, game_state.board)
